...
export default ({ questionnaires, intl: { formatMessage } }) => {
	...
	const actionsCellRenderer = ({ rowData }) => <span>
		<Link style={ style.actionLink } to={ '/questionnaires/edit/' + rowData._id }>{ translatedEdit }</Link>
		
		&nbsp;
		
		<Link style={ style.actionLink } to={ '/questionnaires/answers/' + rowData._id }>{ translatedAnswers }</Link>
		
		&nbsp;
		
		{ rowData.published ?
				<Link style={ style.actionLink }
						to={ '/questionnaires/take/' + rowData._id }>{ translatedTakeQuestionnaire }</Link> : null
		}
	</span>
