...
const emergencyAuthOverride = settings.emergencyAuthOverride;

// fake logged-in user in case the settings say so
if(emergencyAuthOverride) {
	Accounts.registerLoginHandler('sspi', ({ sspi }) => {
		if(!sspi) {
			return undefined;
		}
		
		const rawUsername = 'EMERGENCY\\admin';
		
		const username = rawUsername.toLowerCase();
		
		const role = 'operator';
		
		const adUserProperties = {
			displayName: 'Emergency User',
			description: 'משתמש חירום',
...
// no emergency auth override, handle users the usual secure way
