					...
					{ fields.map((answer, index) => (
							<div style={ this.style.answersContainer } key={ index }>
								<div style={ this.style.numberContainer }>
									<NumberBadge content={ index + 1 } primary={ true }
											style={ this.style.numberBadge } />
								</div>
								<div style={ this.style.checkboxContainer }>
									<Field component={ Checkbox } name={ `${ answer }.correct` }
											style={ this.style.checkbox }
											normalize={ this.normalizeBoolean } iconStyle={ iconStyle } />
								</div>
								<Field component={ TextField } name={ `${ answer }.text` }
										floatingLabelText={ <FormattedMessage id='answer'
								...
								<div style={ this.style.addRemoveAnswerButtonsContainer }>
									<RemoveAnswerButton disabled={ fields.length < 3 }
											onClick={ this.handleRemoveAnswerButtonClick }
											number={ index } />
								</div>
							</div>
					)) }
